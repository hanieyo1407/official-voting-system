...
function initializePool(): Pool {
    ...
    try {
        ...
        pool.on('error', (err) => {
            console.error('⚠️ UNEXPECTED DATABASE ERROR on idle client (Pool will attempt to recover):', err);
        });

        // Immediate connection test (async)
        pool.query('SELECT 1 + 1').catch((err) => {
            console.error('❌ FATAL: Database connection test failed. Check credentials/certificate.', err.message);
            LoggingService.logError(err, { context: 'db_pool_test_failed' });
            process.exit(1); 
        });
        
        console.log('✅ Database Pool Initialized (Connection Test Running in Background)');
        return pool;

    } catch (error: any) {
        console.error('❌ FATAL: Database Pool Initialization Error. Check .env variables or certificate.', error.message);
        process.exit(1);
