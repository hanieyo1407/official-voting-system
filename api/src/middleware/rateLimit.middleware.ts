...
export class CustomRateLimiter {
  ...
  createRateLimitMiddleware(
    ...
    return (req: Request, res: Response, next: any) => {
      ...
      const normalizedIP = ip.replace(/:/g, "_").replace(/\./g, "_");
      const rateLimitCheck = this.checkIPRateLimit(
        normalizedIP,
        maxRequests,
        windowMs,
      );
      ...
      res.set({
        ...
        "X-RateLimit-Remaining": Math.max(
          0,
          maxRequests - rateLimitCheck.currentCount,
        ),
...
export const strictRateLimit = customRateLimiter.createRateLimitMiddleware(
  5,
  60 * 1000,
); // 5 requests per minute
export const moderateRateLimit = customRateLimiter.createRateLimitMiddleware(
  50,
  5 * 60 * 1000,
); // 50 requests per 5 minutes
export const looseRateLimit = customRateLimiter.createRateLimitMiddleware(
  200,
  15 * 60 * 1000,
