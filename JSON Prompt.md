{
  "app_name": "DMI Voting System",
  "description": "A secure, transparent, and accessible electronic voting platform for DMI St John The Baptist University.",
  "core_phases": [
    "Registration Phase",
    "Voting Phase",
    "Verification & Results Phase"
  ],
  "features": {
    "authentication": ["QR-code vouchers", "Email OTP (2FA)", "Optional SMS fallback"],
    "security": ["AES-256 vote encryption", "SHA-256 hashing with blockchain-style integrity", "TLS 1.3 in transit", "CSRF/XSS protection", "Audit logs", "Rate limiting", "Role-based access control"],
    "resilience": ["Offline fallback with resync", "Backup & restore system"],
    "transparency": ["Verification portal (receipt-based)", "Public results dashboard", "Downloadable reports (CSV/PDF)", "Audit trail viewer"]
  },
  "roles": {
    "student_voter": {
      "permissions": ["Register once", "Vote once", "Verify vote", "View results"]
    },
    "registration_official": {
      "permissions": ["Verify IDs", "Register voters", "Issue/reissue vouchers"]
    },
    "poll_worker": {
      "permissions": ["Scan vouchers", "Assist voters", "Report incidents"]
    },
    "election_administrator": {
      "permissions": ["Configure elections", "Monitor stats", "Generate reports", "Manage results"]
    },
    "system_administrator": {
      "permissions": ["Full technical access", "Backups", "Database security", "System updates"],
      "restrictions": ["Cannot access encrypted votes directly"]
    }
  },
  "architecture": {
    "frontend": {
    ...
    "backend": {
      ...
      "sms_service": "Twilio or Africa’s Talking"
    },
    "database": "PostgreSQL or MySQL",
    "infrastructure": {
      "hosting": "Cloud (AWS/DigitalOcean) or local server",
  ...
  "database_schema": {
    "tables": [
      {
        "name": "voters",
        "fields": ["voter_id", "email", "phone", "voucher_code", "qr_hash", "registration_timestamp", "has_voted", "voted_timestamp", "status"]
      },
      {
        "name": "votes",
        "fields": ["vote_id", "position_id", "candidate_id", "encrypted_vote", "vote_hash", "previous_vote_hash", "timestamp", "verification_code"]
      },
      {
        "name": "positions",
        "fields": ["position_id", "position_name", "description", "display_order", "is_active"]
      },
      {
        "name": "candidates",
        "fields": ["candidate_id", "position_id", "full_name", "photo_url", "bio", "manifesto", "display_order"]
      },
      {
        "name": "audit_logs",
        "fields": ["log_id", "timestamp", "event_type", "user_type", "user_id", "action", "details", "ip_address", "status"]
      }
    ]
  },
  "modules": {
    "registration": "Student shows ID, official verifies, system generates voter_id & QR voucher, email confirmation sent, voucher can be reissued if lost.",
    "authentication": "On election day, student presents voucher, QR scanned, system validates, OTP sent to email, student enters code, ballot access granted.",
    "voting": "Ballot displayed per position, one candidate or abstain allowed, review before submission, votes encrypted and hashed, verification code generated, receipt emailed/printed.",
    "results": "Votes tallied automatically, results dashboard with charts, turnout stats, downloadable reports, verification portal using receipt codes.",
    "admin_dashboard": "Real-time statistics, turnout by faculty/station, alerts, incident logs, integrity checks, system monitoring."
  },
  "user_experience": {
    "students": "Simple registration, clear ballot UI, mobile-friendly, receipt-based verification",
    "officials": "Fast voucher issuance & scanning, assisted workflows",
    "admins": "Intuitive dashboards, reports, and monitoring",
    "accessibility": "Supports students without registration numbers, allows personal Gmail/Outlook emails"
  },
  "deliverables": [
    "Web application (responsive)",
    "Optional desktop client for polling stations",
    "Setup & configuration scripts",
    "User manuals & troubleshooting guide",
